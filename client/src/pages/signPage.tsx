...
import { FaUserPlus, FaLock, FaTimes, FaExclamationTriangle, FaUser } from 'react-icons/fa';
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-100 to-purple-100 p-4">
      ...
      {showSuccessNotification && (
        <div className="fixed top-4 right-4 z-50">
          <motion.div
            className="bg-green-500 text-white rounded-lg shadow-lg p-4 w-80 max-w-sm relative overflow-hidden"
            variants={notificationVariants}
            initial="hidden"
            animate="visible"
            exit="exit"
          >
            <div className="flex items-start justify-between mb-2">
              <div className="flex items-center">
                <FaUserPlus className="text-white text-lg mr-3 flex-shrink-0" />
                <div>
                  <h4 className="font-semibold text-sm">Signup Successful</h4>
                  <p className="text-sm text-green-100 mt-1">{message}</p>
                </div>
              </div>
              <button
                onClick={closeSuccessNotification}
                className="text-green-200 hover:text-white transition duration-200 flex-shrink-0 ml-2"
              >
                <FaTimes />
              </button>
            </div>
